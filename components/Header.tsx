...
export function Header({
    ...
    const { showRoleModal, setShowRoleModal, setUserRole, userRole } = useGlobalContext();
    ...
    useEffect(() => {
        const checkUserRegistration = async () => {
            if (isConnected && address) {
                try {
                    const response = await axios.post('http://localhost:5000/api/login', { walletAddress: address });
    ...
    const handleRoleSelect = async (role: string) => {
        try {
            const response = await axios.post('http://localhost:5000/api/register', { walletAddress: address, role });
            ...
            >
                <div className="w-full relative flex flex-row items-center justify-between">
                    ...
                    <div className="flex flex-row items-center gap-4">
                        ...
                        <appkit-button onClick={handleButtonClick} />
                    </div>
                </div>
            </div>
